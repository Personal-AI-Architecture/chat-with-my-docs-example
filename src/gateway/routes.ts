...
import { type StaticAssetsHandler } from "./static";
...
export interface GatewayRoutesConfig {
  ...
  static_handler?: StaticAssetsHandler;
}
...
function mapReindexErrorCode(error: unknown): ReindexFailureCode {
  const value = (error as { reindex_code?: unknown })?.reindex_code;
  if (
    value === "folder_missing" ||
    value === "not_a_directory" ||
    value === "memory_write_failed" ||
...
function safeReindexErrorMessage(code: ReindexFailureCode): string {
  switch (code) {
    case "folder_missing":
      return "Configured notes folder is not available.";
    case "not_a_directory":
      return "Configured notes path is not a directory.";
    case "memory_write_failed":
      return "Failed to persist the index in memory.";
    default:
...
export function createGatewayRoutes(config: GatewayRoutesConfig): GatewayRoutes {
  ...
  async function routeMessage(
    ...
    let systemPrompt: CanonicalMessage | null = null;
    if (config.system_prompt_builder) {
      try {
        systemPrompt = await config.system_prompt_builder.build();
      } catch (error) {
        emitAuditLog({
          category: "gateway",
          action: "system_prompt_build_failed",
          outcome: "failure",
    ...
    const engineMessages = systemPrompt
  ...
  async function routeReindex(request: GatewayRouteRequest): Promise<GatewayRouteResponse> {
    const actorHeaders = extractActorHeaders(request.headers);
    if (!actorHeaders) {
      return createJsonResponse(401, {
        error: {
    ...
    if (!config.reindex_handler) {
      return createJsonResponse(503, {
        error: {
          code: "reindex_unavailable",
          message: "Re-index is not configured for this runtime."
        }
      });
    }

    const correlationId = createIdentifier("corr");

    try {
      const result: ReindexResult = await config.reindex_handler.reindex(correlationId);
      emitAuditLog({
        category: "gateway",
        action: "reindex",
        outcome: "success",
      ...
      const code = mapReindexErrorCode(error);
      return createJsonResponse(500, {
        error: {
          code,
          message: safeReindexErrorMessage(code)
        }
      });
    }
  }
  ...
  return {
    async handle(request: GatewayRouteRequest): Promise<GatewayRouteResponse> {
      ...
      if (method === "POST" && path === "/reindex") {
        return routeReindex(request);
      }

      if (config.static_handler && method === "GET") {
        const staticResponse = await config.static_handler.tryServe(method, path);
        if (staticResponse) {
